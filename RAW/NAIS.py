...
class NAIS:
    ...
    def __init__(self, num_items, args):
        ...
        self.logAttention = args.logAttention
        regs = eval(args.regs)
    ...
    def _attention_MLP(self, q_):
       with tf.name_scope("attention_MLP"):
            ...
            self.A = tf.div(exp_A_, exp_sum) # (b, n)
            A = tf.expand_dims(self.A, 2) # (b, n, 1)
...
def training(flag, model, dataset,  epochs, num_negatives, attention_filename):
    ...
    weight_path = 'Pretraining/%s/embed_size%d' % (model.dataset_name, model.embedding_size)
    ...
    if model.ckpt:
        ...
        ckpt_path = "Model_weights/%s/%s/embed_size%d" % (model_name, model.dataset_name, model.embedding_size) 
    ...
    with tf.Session() as sess:
        ...
        for epoch_count in range(epochs):
            ...
            if epoch_count % model.verbose == 0:
                ...
                eval_begin = time()
                (hits, ndcgs, losses, attention) = evaluate.eval(model, sess, dataset.testRatings, dataset.testNegatives, testDict)
                ...
                if model.logAttention:
                    weight_file_mean = open(attention_filename + ('epoch%d' % epoch_count) + "mean", 'w')
                    weight_file_var = open(attention_filename + ('epoch%d' % epoch_count) + "var", 'w')
                    ...
                    weight_file_mean_ = open(attention_filename + ('epoch%d' % epoch_count) + "mean_not", 'w')
                    weight_file_var_ = open(attention_filename + ('epoch%d' % epoch_count) + "var_not", 'w')
...
if __name__=='__main__':
    ...
    filename = "Log/%s/log_beta%.2f_pre%dweight_size%d_reg%.7f_%s" % (args.dataset, args.beta, args.pretrain, args.weight_size, regs[2], strftime('%Y-%m-%d%H:%M:%S', localtime()))
    logging.basicConfig(filename = filename, level = logging.INFO)

    attention_filename = filename + "attention"
    weight_file = open(attention_filename,'w')

    if args.algorithm == 0:
        logging.info("begin training NAIS_prod model ......")
        print >> weight_file, "begin training NAIS_prod model ......"
    else:
        logging.info("begin training NAIS_concat model ......")
        print >> weight_file, "begin training NAIS_concat model ......"

    logging.info("dataset:%s  pretrain:%d   weight_size:%d  embedding_size:%d" % (args.dataset, args.pretrain,  args.weight_size, args.embed_size))
    print >> weight_file, "dataset:%s  pretrain:%d   weight_size:%d  embedding_size:%d" % (args.dataset, args.pretrain,  args.weight_size, args.embed_size)
    logging.info("regs:%.8f, %.8f, %.8f  beta:%.1f  learning_rate:%.4f  train_loss:%d  activation:%d" % (regs[0], regs[1], regs[2], args.beta, args.lr, args.train_loss, args.activation)) 
    print >> weight_file, "regs:%.8f, %.8f, %.8f  beta:%.1f  learning_rate:%.4f  train_loss:%d  activation:%d" % (regs[0], regs[1], regs[2], args.beta, args.lr, args.train_loss, args.activation) 
    ...
    model.build_graph()
    training(args.pretrain, model, dataset, args.epochs, args.num_neg, attention_filename)
